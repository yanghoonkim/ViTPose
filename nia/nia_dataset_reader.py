...
class NiaDataPathExtractor:
    ...
    def pair(self) -> pl.DataFrame:
        ...
        paired_df = self.matched_df.group_by("stem", "channel").agg(
            pl.all().exclude("filename", "path", "extension", "type").first(),
        ...
        #     "sensor", "scene", "road", "timeslot", "weather",
        # ]
    ...
    # TODO: filter method should be refactored to external function
...
class NiaDataPathProvider:
    ...
    def get_split_data_list(self, split: str):
...
class NiaKeypointDataPathProvider:
    def __init__(
            self,
            visible_reader: NiaDataPathExtractor,
            keypoint_reader: NiaDataPathExtractor,
            splitter: DataFrameSplitter,
            channels: List[str]=["image_B", "image_F", "image_L", "image_R"],
        ) -> None:
        self.visible_reader = visible_reader
        self.keypoint_reader = keypoint_reader
        self.splitter = splitter
        self.channels = channels
    
    def get_split_data_list(self, split: str):
        visible_df = self.visible_reader.filter_channels(self.channels)
        keypoint_labels_df = self.keypoint_reader.matched_df.filter(
            pl.col("channel") == "keypoint",
        )

        df = visible_df.join(keypoint_labels_df.select(
            pl.col("stem"),
            pl.col("path").name.prefix("keypoint_"),
        ), on="stem")
